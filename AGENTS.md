...
- **CFW does not need `InitQHYCCD`**: the vendored SDK sample (`testapp/common/ControlCFW.cpp`) opens the camera and calls `SendOrder2QHYCCDCFW`/`GetQHYCCDCFWStatus` directly, with no `InitQHYCCD` in between — confirmed against the SDK's own reference usage. The filter wheel driver therefore only calls `open_camera()`, not `init_camera()`, so it can connect and move the wheel even if the camera driver (which does call `init_camera()`) is never connected in the same session.
...
Devices: CoverCalibrator (the project's first — the generic CoverCalibrator dispatch already existed in `AlpacaHTTP/src/http/router.cpp`; only the vendor instantiation block + web UI were new). The WandererCover V4 is a motorized dust cover combined with an EL flat panel.
