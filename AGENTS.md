...
- **Master-enable is ACTIVE-LOW** (kernel index 3, GPIO0_B7): boots at `level=0` (enabled) per the device-tree inventory at `rk-flashtool/hardware/asiair-plus-rk3568-256g/inventory.md`. Driving it HIGH **disables all four DC ports simultaneously** — that's a global power cut, not a switch enable. `AsiairPlusProtocolWrapper::open()` writes `level=0` here to make sure the active state is restored, even if a previous buggy session left it HIGH. The early dev version of this driver had the polarity inverted (drove it HIGH thinking that "enabled" it) and exhibited a textbook symptom: gear plugged into the DC ports went dark the moment the ASCOM client connected, while the wrapper's cached per-port values still reported ON. Empirically verified: SET_LEVEL on indices 4–7 has no effect on physical 12V output while index 3 is HIGH. Don't repeat the mistake — `level=0` is correct.
