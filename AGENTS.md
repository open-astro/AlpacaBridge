...
- **`open()` itself is read-only** — it opens the fd and touches no kernel-side state. Two early attempts to write anything from *inside* `open()` — driving the master-enable line at kernel index 3, and separately setting mode+level on each DC port — both caused every DC port to physically drop to 0V the moment an ASCOM client connected. `open()` therefore only opens the fd; the constructor's optimistic cache (value=1/100 = "on") is what a fresh connect reports before any port-level write happens.

- **Every port defaults OFF as soon as AlpacaBridge starts, including across a reboot (issue #300).**
  - **Original behavior and the problem**: a port's power state came entirely from the hardware — the RK3568's boot-time pull-ups drive all 4 DC lines HIGH before AlpacaBridge even starts, and the driver never wrote an initial value of its own. A dew heater or flat panel could therefore be silently live at full power indefinitely after any reboot, with no client having asked for it.
  - **Fix**: `ZWOAsiairPlusSwitchDriver::apply_default_off_ports()` writes each DC port OFF individually (never the master-enable line at index 3) right after a fresh `wrapper_.open()` succeeds, in `set_connected(true)`. Since a persisted device is only *constructed* at process startup — `load_persisted_devices()` never calls `connect()` on anything it loads — `Router::register_device_from_config` (`asiair-plus-rk3568` branch, `AlpacaHTTP/src/http/router.cpp`) also runs one synchronous connect+disconnect cycle immediately when constructing the device from `registered_devices.json` (`ConfigSource::Persisted`). This applies the default-off write the moment the process starts, independent of whether any client ever connects, then leaves the device correctly ASCOM-disconnected — the kernel module retains each port's level across an fd close, so the physical state stays OFF.
  - **Boolean ports turn off synchronously** — `set_value()` writes the ioctl inline, so by the time the write loop above returns, those ports are already physically off. **PWM-enabled ports do not**: `set_value()` only updates an atomic that a background worker thread applies to hardware on its next ~20ms poll cycle, so `apply_default_off_ports()` sleeps 60ms after the write loop whenever any port is PWM-enabled — otherwise the boot-time connect+disconnect cycle above could tear that thread down before it ever applies the write. This wait only matters for PWM ports; it's a no-op for an all-boolean config.
  - **Validated on real RK3568 hardware**: individual and multi-port writes at connect time confirmed only the targeted ports change (the two failures above are specific to writing from inside `open()`, not to this pattern); a genuine reboot confirmed the port LEDs come on at the hardware default and go dark within seconds with no client connecting; ConformU 4.5.1 re-validated 0 errors / 0 issues / 0 timing on a 4-port all-PWM config, report at `AlpacaCore/conformu/ZWO/ASIair Plus (RK3568)/Linux-arm64.txt`. Unconditional for every port, by product decision — no config option.
