...
- **`open()` itself is still read-only** — it opens the fd and touches no kernel-side state. Two early attempts to "initialize" the kernel module from *inside `open()`* — first driving the master-enable line at kernel index 3, then explicitly setting mode + level on each DC port — both produced the same symptom: every DC port physically dropped to 0 V the moment an ASCOM client connected. `open()` therefore still just opens the fd and does nothing else; the constructor's optimistic cache (value=1 / 100 = "on") is what a fresh connect reports before any port-level write happens.

- **Every port now defaults OFF once, right after connect (issue #300, resolved 2026-09-11)** — but this is done by the *driver*, one layer above `open()`, not inside it, and only for the individual DC port lines, never the master-enable line at kernel index 3. Before this fix, ports came up however the hardware/previous session left them (often boot-default ON), which meant a dew heater or flat panel could be silently live at full power after every reboot. `ZWOAsiairPlusSwitchDriver::apply_default_off_ports()` runs immediately after a **fresh** `wrapper_.open()` succeeds (never on an idempotent already-open connect) and calls `wrapper_.set_value(i, 0)` — the same SET_MODE→ENABLE→SET_LEVEL sequence a client's `SetSwitch(false)` would issue — for each of the 4 DC ports individually. **This looks exactly like the historical failure mode #2 above (per-port SET_MODE/SET_LEVEL at connect) and was re-tested specifically because of that history**, not assumed safe by analogy: on the live RK3568 unit (2026-09-11), a single-port immediate write (Port 1 off, 0ms delay after open) left Ports 2–4 lit; a multi-port immediate write (Ports 1+2 off back-to-back, 0ms delay) left Ports 3–4 lit; both were then reversed and reconfirmed restored, and finally validated end-to-end through the real driver (`PUT switch/1/connected` → all 4 report `GetSwitch=false`, manual `SetSwitch` still works normally afterward). The historical failure did not reproduce in any of these trials. Working theory for the discrepancy: failure #1 (master-enable, kernel index 3) is a distinct line from the DC ports and plausibly still hazardous — this fix never touches it; failure #2 may have been touching multiple/different ports, a different kernel module build, or a load-dependent transient not present with nothing (or only benign gear) attached during this validation. **If this is ever revisited, re-run the same single-port / multi-port / restore protocol on real hardware before trusting a code-only argument that it's safe** — do not reason your way back into the master-enable line, and do not skip re-testing on the strength of this note alone if the kernel module, firmware, or hardware revision changes. Best-effort: a failed default-off write on one port is logged (WARN) and does not fail the connect or block the other ports. No config option exists for this — it is unconditional for every port on every connect, by explicit product decision (not a per-port opt-in).
