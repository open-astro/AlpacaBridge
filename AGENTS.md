...
- **The kernel module needs SET_MODE → ENABLE → SET_LEVEL.** Reverse-engineered the hard way after observing that toggles in NINA had zero physical effect even though every ioctl returned 0. The kernel debug interface revealed the DC port GPIO lines (GPIO 146/147/149/150 on gpiochip4) were stuck as **inputs** with external pull-ups holding them HIGH, regardless of how many `SET_LEVEL` calls our driver made. The kernel module silently rejects writes to lines that aren't in OUTPUT direction (it logs `"NOT in GPIO OUTPUT mode"` via printk but returns 0 to userspace). The fix: call `PWM_GPIO_ENABLE` between `SET_MODE` and `SET_LEVEL` — internally that maps to `gpiod_direction_output_raw` and flips the line. Verified end-to-end with a flat-panel load on DC port 2: SET_MODE(GPIO) → ENABLE → SET_LEVEL(0) cuts power, SET_LEVEL(1) restores it. Active-high.

- **PWM mode accepts ioctls but does not visibly dim on stock Debian.** The same SET_MODE → ENABLE → SET_CONFIG sequence works at the kernel level (no errors, `GET_CONFIG` echoes back the period/duty we wrote). But a 12V LED load shows no visible duty-cycle response at any frequency tried (1 kHz tested with 10%/50%/100%). The .ko symbol table includes `hrtimer_init` / `hrtimer_start_range_ns` so the module *can* generate timer-driven toggles in principle; on ZWO's own ASIair OS the `zwoair_imager` / `zwoair_guider` daemons (not present on stock Debian) presumably perform additional setup (possibly `pinctrl_select_state` to mux the pad to a real PWM controller). Until that piece is identified, the driver sends the full PWM sequence anyway — so it "just starts working" if/when the missing piece lands — but PWM is advertised as **not yet functional** in `PowerPorts.md` and `SUPPORTED-DRIVERS.md`. Don't waste hours re-probing duty-cycle math or alternative ioctl orderings; all succeed at the ioctl level and none affect the load.
...
- **PWM is kernel-side hardware, not userspace soft-PWM**: unlike the Pi 4 ASIair Pro driver, the RK3568 driver does **not** spawn worker threads. The kernel module's hrtimer handles the period+duty toggling directly — we just issue one `PWM_GPIO_SET_CONFIG` ioctl per duty change. Empirically validated against the module across 100 Hz / 1 kHz / 10 kHz / 40 kHz / 100 kHz: every readback exactly matched the requested `period_ns`/`duty_ns`. The kernel hrtimer is well-behaved across the full range. Defaults to 1 kHz; range 1–100,000 Hz configurable via `pwmFrequencyHz`.
