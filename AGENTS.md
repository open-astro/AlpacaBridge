...
- **Clockwise-only direction**: The SDK supports `direction=0` (clockwise) and `direction=1` (auto). Auto-direction has a firmware bug on moves that cross the 0 boundary counterclockwise (e.g., 0→6, 3→0). These cause >30s ConformU timeouts. Clockwise-only is slower for some moves but is the only reliably detected direction. Use direction=0 for all moves.
