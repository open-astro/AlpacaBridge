...
- **Auto-direction with timeout on 0→N-1**: The SDK supports `direction=0` (clockwise) and `direction=1` (auto). Auto-direction picks the shortest path and works correctly for most moves (~0.2s/slot). However, it has a firmware bug on the 0→N-1 farthest move (e.g., 0→6 on a 7-slot wheel) causing >30s timeouts. This is a firmware boundary bug, not a code defect. Since auto-direction is faster for all other moves and the 0→N-1 timeout is identical with clockwise-only, auto-direction is preferred.
