...
- **Master-enable auto-set on connect**: `AsiairPlusProtocolWrapper::open()` sets kernel index 3 (master enable) to HIGH unconditionally. The kernel module already configures it for output via the airplus-gpios pinctrl, so we only need the level write. Without this the per-port toggles look like they work but no current flows.
