...
- **Smart direction**: The driver calculates the shortest path (CW vs CCW) and uses auto-direction (`direction=1`) when clockwise is the shortest or equal path. When counterclockwise would be shorter, it forces clockwise (`direction=0`) to avoid a firmware bug where CCW moves crossing the 0 boundary (e.g., 0→6, 3→0) cause >30s timeouts.
