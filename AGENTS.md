...
Devices: Camera, Focuser (AAF — Astro Auto Focuser), FilterWheel (AFW — Astro Filter Wheel, AFW-M 5/7-slot), Switch (StellaVita PowerBox).

SDK location: `AlpacaCore/external/ToupTek/toupcamsdk.20260128/` (shared between camera, focuser, and filter-wheel drivers). The StellaVita Switch driver uses **no SDK** — it is a libgpiod-only driver that happens to live under the ToupTek vendor.

- **Single SDK, three device types**: Camera, focuser, and filter-wheel drivers all go through `ToupTekSDKWrapper`. Cameras enumerate via `enumerate_cameras()`, focusers via `enumerate_focusers()` (filters `Toupcam_EnumV2` by `TOUPCAM_FLAG_AUTOFOCUSER`), filter wheels via `enumerate_filter_wheels()` (filters by `TOUPCAM_FLAG_FILTERWHEEL`). Same `Toupcam_Open` is used for all three — the device-class is determined entirely by the capability flag.
