...
- **Auto-direction**: Always use `direction=1` (auto). The SDK's auto-direction works correctly for all moves where clockwise is the shorter path. Moves where counterclockwise is shorter at the 0/N boundary (e.g., 0→6 on a 7-slot wheel) can time out (>30s) due to a firmware bug. This is a hardware limitation — the driver cannot work around it without breaking the async contract.
