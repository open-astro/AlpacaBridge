...
- **open() is read-only — do NOT touch any kernel-side state on connect.** Two earlier attempts to "initialize" the kernel module from open() — first driving the master-enable line at kernel index 3, then explicitly setting mode + level on each DC port — both produced the same symptom: every DC port physically dropped to 0 V the moment an ASCOM client connected, while our cached per-port values continued to report ON. The polarity of index 3 (`"DC master enable / control signal"`) is documented ambiguously, and `SET_MODE`/`SET_LEVEL` on the DC ports themselves appear to introduce a brief transient that real gear (cameras, mounts) notices and reacts to. The kernel module is closed-source / reverse-engineered, so we cannot debug this from first principles. The accepted policy is therefore: `open()` reads each DC port's current level via `PWM_GPIO_GET_LEVEL` into our cache (so the client UI shows reality, not a default) and writes nothing. The first hardware write happens only when the ASCOM client calls `SetSwitch` or `SetSwitchValue`, at which point `apply_port_locked` issues `SET_MODE` then `SET_LEVEL` (or `SET_CONFIG`). A brief mode-change transient on the *user-initiated* write is acceptable; the same transient on every connect was not. Do not "fix" this by adding any kind of init-time write — the symptom returns immediately and is genuinely hard to debug because `GET_LEVEL` continues to lie about the physical state.
