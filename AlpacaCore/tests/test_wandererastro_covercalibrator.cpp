...
TEST_CASE("WandererAstro CoverCalibrator Driver - Defaults",
          "[wandererastro][covercalibrator][unit]") {
    auto driver = alpacacore::vendor::wandererastro::create_wandererastro_covercalibrator(
        0, "/dev/ttyUSB0");
...
TEST_CASE("WandererAstro CoverCalibrator Driver - Device metadata",
          "[wandererastro][covercalibrator][unit]") {
    auto driver = alpacacore::vendor::wandererastro::create_wandererastro_covercalibrator(
        3, "/dev/ttyUSB0");
    ...
    CHECK(driver->get_description() ==
          "WandererAstro WandererCover V4 CoverCalibrator Driver");
...
TEST_CASE("WandererAstro CoverCalibrator Driver - Not connected throws",
          "[wandererastro][covercalibrator][unit]") {
    auto driver = alpacacore::vendor::wandererastro::create_wandererastro_covercalibrator(
        0, "/dev/ttyUSB0");
    ...
    require_alpaca_error([&]() { driver->get_brightness(); },
                         alpacacore::AlpacaError::NotConnected);
    require_alpaca_error([&]() { (void)driver->get_calibrator_state(); },
                         alpacacore::AlpacaError::NotConnected);
    require_alpaca_error([&]() { (void)driver->get_calibrator_changing(); },
                         alpacacore::AlpacaError::NotConnected);
    require_alpaca_error([&]() { (void)driver->get_cover_state(); },
                         alpacacore::AlpacaError::NotConnected);
    require_alpaca_error([&]() { (void)driver->get_cover_moving(); },
                         alpacacore::AlpacaError::NotConnected);
    require_alpaca_error([&]() { driver->calibrator_off(); },
                         alpacacore::AlpacaError::NotConnected);
    require_alpaca_error([&]() { driver->open_cover(); },
                         alpacacore::AlpacaError::NotConnected);
    require_alpaca_error([&]() { driver->close_cover(); },
                         alpacacore::AlpacaError::NotConnected);
    require_alpaca_error([&]() { driver->halt_cover(); },
                         alpacacore::AlpacaError::NotConnected);
    // A valid-brightness CalibratorOn passes range validation, then trips on the
    // connection check.
    require_alpaca_error([&]() { driver->calibrator_on(128); },
                         alpacacore::AlpacaError::NotConnected);
}

TEST_CASE("WandererAstro CoverCalibrator Driver - Unsupported actions",
          "[wandererastro][covercalibrator][unit]") {
    auto driver = alpacacore::vendor::wandererastro::create_wandererastro_covercalibrator(
        0, "/dev/ttyUSB0");
...
TEST_CASE("WandererAstro CoverCalibrator Driver - Calibrator capabilities",
          "[wandererastro][covercalibrator][unit]") {
    auto driver = alpacacore::vendor::wandererastro::create_wandererastro_covercalibrator(
        2, "/dev/ttyUSB0");
    ...
    require_alpaca_error([&]() { driver->set_brightness(10); },
                         alpacacore::AlpacaError::NotConnected);
    require_alpaca_error([&]() { driver->set_brightness(-5); },
                         alpacacore::AlpacaError::InvalidValue);
}

TEST_CASE("WandererAstro CoverCalibrator Driver - Value range validation",
          "[wandererastro][covercalibrator][unit]") {
    auto driver = alpacacore::vendor::wandererastro::create_wandererastro_covercalibrator(
        0, "/dev/ttyUSB0");

    // Out-of-range brightness must throw InvalidValue (0x401), not normalize.
    require_alpaca_error([&]() { driver->calibrator_on(-1); },
                         alpacacore::AlpacaError::InvalidValue);
    require_alpaca_error([&]() { driver->calibrator_on(256); },
                         alpacacore::AlpacaError::InvalidValue);
    require_alpaca_error([&]() { driver->calibrator_on(1000); },
                         alpacacore::AlpacaError::InvalidValue);

    // In-range boundary values are accepted by validation, then hit NotConnected.
    require_alpaca_error([&]() { driver->calibrator_on(0); },
                         alpacacore::AlpacaError::NotConnected);
    require_alpaca_error([&]() { driver->calibrator_on(255); },
                         alpacacore::AlpacaError::NotConnected);
}

TEST_CASE("WandererAstro CoverCalibrator Driver - State machine",
          "[wandererastro][covercalibrator][unit]") {
    auto driver = alpacacore::vendor::wandererastro::create_wandererastro_covercalibrator(
        0, "/dev/ttyUSB0");
...
TEST_CASE("WandererAstro CoverCalibrator Driver - HaltCover is implemented",
          "[wandererastro][covercalibrator][unit]") {
    auto driver = alpacacore::vendor::wandererastro::create_wandererastro_covercalibrator(
        0, "/dev/ttyUSB0");
