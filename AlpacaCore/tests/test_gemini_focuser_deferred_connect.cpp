...
// Gemini focuser: auto-detect resolves at connect time, not at construction
...
using Info = alpacacore::vendor::gemini::ConnectionConfig;

using Fake = alpacacore::test::FakeGeminiFocuser;

Info endpoint(const std::string& path) {
    Info config;
    config.type = alpacacore::vendor::gemini::ConnectionType::Serial;
...
alpacacore::test::DeferredFactory<Info> make_driver() {
    return [](alpacacore::util::ConnectionResolver<Info> resolver) -> std::unique_ptr<alpacacore::AlpacaDriver> {
        return alpacacore::vendor::gemini::create_gemini_focuser_deferred(0, std::move(resolver));
    };
}

}  // namespace

TEST_CASE("Gemini focuser auto-detect - a failed scan refuses the connect, not construction",
          "[gemini][focuser][unit]") {
    alpacacore::test::check_deferred_connect_refused<Info>(make_driver(), "nothing answered the auto-detect probe");
}

TEST_CASE("Gemini focuser auto-detect - resolves at connect and reuses the endpoint", "[gemini][focuser][unit]") {
    std::unique_ptr<Fake> fake;
    alpacacore::test::check_deferred_connect_reuses_endpoint<Info>(make_driver(), [&fake] { return spawn(fake); });
}

TEST_CASE("Gemini focuser auto-detect - re-scans when the resolved endpoint dies", "[gemini][focuser][unit]") {
    std::unique_ptr<Fake> fake;
    alpacacore::test::check_deferred_connect_re_resolves<Info>(make_driver(), [&fake] { return spawn(fake); });
}

TEST_CASE("Gemini focuser auto-detect - the production factory constructs with no hardware",
          "[gemini][focuser][unit]") {
    {
        ...
        REQUIRE_NOTHROW(driver = alpacacore::vendor::gemini::create_gemini_focuser_by_index(0, 0));
