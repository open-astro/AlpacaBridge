...
// OnStep telescope: auto-detect resolves at connect time, not at construction
...
using Info = alpacacore::vendor::onstep::ConnectionInfo;

using Fake = alpacacore::test::FakeMountServer;

// The canned "0#" default responder is enough for connect (see
...
alpacacore::test::DeferredFactory<Info> make_driver() {
    return [](alpacacore::util::ConnectionResolver<Info> resolver) -> std::unique_ptr<alpacacore::AlpacaDriver> {
        return alpacacore::vendor::onstep::create_onstep_telescope_deferred(0, std::move(resolver));
    };
}

}  // namespace

TEST_CASE("OnStep telescope auto-detect - a failed scan refuses the connect, not construction",
...
TEST_CASE("OnStep telescope auto-detect - resolves at connect and reuses the endpoint", "[onstep][telescope][unit]") {
    std::unique_ptr<Fake> fake;
    alpacacore::test::check_deferred_connect_reuses_endpoint<Info>(make_driver(), [&fake] { return spawn(fake); });
}

TEST_CASE("OnStep telescope auto-detect - re-scans when the resolved endpoint dies", "[onstep][telescope][unit]") {
    std::unique_ptr<Fake> fake;
    alpacacore::test::check_deferred_connect_re_resolves<Info>(make_driver(), [&fake] { return spawn(fake); });
}

TEST_CASE("OnStep telescope auto-detect - the production factory constructs with no hardware",
          "[onstep][telescope][unit]") {
    {
        ...
        REQUIRE_NOTHROW(driver = alpacacore::vendor::onstep::create_onstep_telescope_auto(0, 0));
