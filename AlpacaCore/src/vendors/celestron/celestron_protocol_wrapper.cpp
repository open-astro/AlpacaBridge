...
// unrelated FTDI dongles, GPS receivers, or another mount controller sharing
// the ttyUSB namespace -- not just Celestron-shaped USB-serial hardware.
// Filters the raw fallback exactly as narrowly as the by-id loop below.
bool raw_port_looks_like_celestron_candidate(const std::string& port_path) {
...
std::vector<CelestronPortInfo> enumerate_celestron_ports() {
    ...
    if (alpacacore::util::path_exists(serial_by_id)) {
        for (const auto& sym : alpacacore::util::list_serial_by_id(serial_by_id)) {
            ...
            ALPACA_LOG_INFO("Celestron", probe_message);

            std::string fw = probe_nexstar_port(resolved);
            if (!fw.empty()) {
                std::string found_message = "Found NexStar mount on ";
                found_message.append(resolved).append(" (HC firmware ").append(fw).append(")");
                ALPACA_LOG_INFO("Celestron", found_message);
    ...
    // (used by Celestron's serial cable, and commonly also by other devices
    // on the same box) can report identical descriptor strings with no
    ...
    for (int i = 0; i < 10; ++i) {
        ...
        if (!raw_port_looks_like_celestron_candidate(resolved)) continue;
        probed.insert(resolved);
