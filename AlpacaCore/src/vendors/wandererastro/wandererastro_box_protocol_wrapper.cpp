...
// ttyUSB namespace -- not just CH340/CH341-class box hardware. Filters the
// raw fallback exactly as narrowly as the by-id loop below.
bool raw_port_looks_like_box_candidate(const std::string& port_path) {
...
std::vector<BoxPortInfo> enumerate_wandererbox_ports() {
    ...
    // this box, and commonly also by other devices on the same box) report
    ...
    for (int i = 0; i < 10; ++i) {
        ...
        if (!raw_port_looks_like_box_candidate(resolved)) continue;
        probed.insert(resolved);

        if (is_serial_port_in_use(resolved)) continue;  // held by another connected device
        ALPACA_LOG_INFO("WandererAstro", "Probing " + resolved + " for WandererBox Pro V3...");
        BoxPortInfo info;
        if (probe_box_port(resolved, info)) {
            ALPACA_LOG_INFO("WandererAstro", "Found " + info.model + " on " + resolved + " (firmware " +
                                                 std::to_string(info.firmware_version) + ")");
