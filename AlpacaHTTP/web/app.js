...
// open-astro#289: persist the client-clock policy through the description PUT.
async function updateSyncClockFromClients(enabled) {
    setServerInfoStatus('Updating clock policy...');
    try {
        const response = await fetch(API_BASE + '/management/v1/description', {
            method: 'PUT',
            headers: {'Content-Type': 'application/json'},
            body: JSON.stringify({SyncSystemClockFromClients: !!enabled})
        });
        if (!response.ok) {
            throw new Error(`HTTP error! status: ${response.status}`);
        }
        ...
        setServerInfoStatus(enabled ? 'Clients may set the clock when NTP is absent.' : 'Clients will not touch the clock.');
    } catch (e) {
...
async function refreshServerClockOffset() {
    try {
        ...
        if (result && result.ErrorNumber === 0 && Number.isFinite(result.Value)) {
            ...
            // without it a non-numeric Value makes serverClockOffsetMs NaN, which
            // passes the `=== null` guard in updateServerClock(). From
...
// The live server clock's formatting helpers (localZoneLabel,
// formatServerClock) live in web/format.js, which index.html loads first, so
// they can be unit-tested from Node without a DOM stub (open-astro#385).
