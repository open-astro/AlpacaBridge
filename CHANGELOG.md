...
- **QHY Filter Wheel Support** (AlpacaHTTP): router registration (`qhy` + `filterwheel`), web UI CFW config section (camera index/id bound to the paired camera, slot-count picker matching the SDK's 6/7/8/9-slot lineup, filter-name editor), config persistence fix (the `qhy` sanitizer branch didn't allowlist `filterNames`, which would have silently dropped custom filter names on save), and a routing round-trip test.
