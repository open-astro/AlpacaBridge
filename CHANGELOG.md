...
### Fixed
- **Gemini Astro Automatic FlatPanel v2 re-validated on real hardware** (ConformU 4.5.0, up from the 4.4.0 used at the previous validation): 0 errors, 0 issues, run against a standalone AlpacaHTTP instance on the arm64 target (astro, real panel on `/dev/ttyUSB0`) rather than a remote x86 client as the previous log recorded. 4 FAST-target timing warnings (`DeviceState`, `CalibratorState`, `CoverState`, `MaxBrightness`), all clustered in the first ~1.5s after `Connect()` — the same accepted connect-adjacent cold-start window already documented for OnStep; no code changes were made. Results in `AlpacaCore/conformu/Gemini/Astro Automatic FlatPanel v2/Linux-arm64.txt`.
