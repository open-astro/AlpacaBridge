...
- **ZWO ASIair Plus (RK3568) Switch Driver** (AlpacaCore): new `SwitchDriver` implementation for the Rockchip RK3568 variant of the ASIair Plus. Talks to ZWO's custom `pwm_gpio.ko` kernel module via ioctls on `/dev/pwm-gpio-misc` (header reverse-engineered and vendored at `AlpacaCore/external/ZWO/asiair-plus/pwm_gpio.h`). Exposes the four 12V DC ports as ASCOM Switch channels with per-port boolean on/off or **hardware** PWM via the kernel hrtimer at a configurable frequency (default 1 kHz, empirically validated 100 Hz – 100 kHz). Wrapper indices 0–3 map to kernel ioctl indices 4–7 (DC1–DC4); master-enable (kernel index 3) auto-driven HIGH on connect. Disconnect-leaves-state policy mirrors the Pi 4 ASIair Pro driver. Independent driver from the libgpiod-based `asiair` — the kernel-interface differences are fundamental.
...
### Changed
- **ZWO ASIair Plus (RK3568) soft-PWM default frequency lowered to 200 Hz** (AlpacaCore): the wrapper's default `pwm_frequency_hz` is now 200 Hz, down from the original 1 kHz target. Tuned empirically against real hardware sweeping 80 Hz – 20 kHz: 1 kHz produced an audible whine from the ASIair Plus's own MOSFET driver (human ear sensitivity peaks at 2–4 kHz so the original default was the worst-case band); 500 Hz was noticeably quieter; 200 Hz was the quietest setting that stayed stable across every retest. Going lower (≤ 100 Hz) showed intermittent flicker depending on scheduler load, and 80 Hz caused load instability when the connected panel's smoothing cap discharged too far during the off-phase. Hardware constraint documented inline: the RK3568 has 16 hardware PWM peripherals but NONE of them have pinctrl entries for the airplus-gpios pins on GPIO bank 4 — so software PWM is the only path on this board (confirmed against the extracted stock `zwoair_imager` daemon binary, which takes the same approach). Caveat also captured for future contributors: the test load was an HSK A4 LED tracing pad with its own internal touch-dimming PWM controller, so cascaded-PWM interference inflated the observed flicker thresholds above what a passive flat-field panel would show.
