...
### Added
- **High-speed planetary video capture engine, phase 1** (AlpacaCore): the vendor-neutral core of planetary/lucky SER recording (openastro-ara playbook §77.1) — a `VideoCapture` seam (`start/get_frame/stop/dropped_frames`, one implementation per vendor SDK), a pre-allocated single-producer/single-consumer RAM ring buffer with §77.1 adaptive sizing (`clamp(MemAvailable/4, 64 MB, 512 MB)`, no allocation in the hot path), a SER v3 writer (178-byte header, raw frames, per-frame UTC timestamp trailer; Linux `O_DIRECT` through an aligned staging buffer so page-cache writeback can't eat a 2 GB box's RAM mid-capture), a `VideoRecorder` orchestrating capture→ring→disk with honest drop accounting (ring drops and SDK drops counted separately; `frames_captured == frames_written + ring_dropped` always), a sustained-disk-rate probe, and ZWO glue over `ASIStartVideoCapture`/`ASIGetVideoData`/`ASIGetDroppedFrames` as the first vendor behind the seam. Bench-tested hardware-free via a synthetic frame source (15 test cases incl. a `[stress]` start/stop storm). Mode arbitration vs Alpaca still imaging, usbfs auto-tune, and the HTTP surface are the next phases — nothing user-visible changes yet.
