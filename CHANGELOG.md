...
- **ZWO ASIair Pro Device Support** (AlpacaHTTP): router accepts `vendor=zwo deviceType=switch switchType=asiair` with optional `gpioChip`, `pwmFrequencyHz`, and `ports[]` overrides for non-Pi-4 SBC deployments. Web UI Switch Type dropdown adds the new option and hides the camera-binding fields when ASIair is selected.
