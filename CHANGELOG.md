...
- **Web UI: editing a device could silently resubmit and persist a stale cached config** (AlpacaHTTP `app.js`): `handleEditDeviceClick` populated the Edit form from `currentDevices`, which is only refreshed on page load or after a submit from that same tab — a browser tab left open across an out-of-band config change (another tab, a direct management-API call) would still show the old config, and clicking Edit → Save silently overwrote the newer change. The device list is now re-fetched before the Edit form opens.
