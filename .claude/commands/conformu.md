...
**Site.** Motor-controller mounts (Sky-Watcher) store no site. Since PR #353 the driver refuses `Connected = true` with `InvalidOperation` until both site latitude and longitude have been set (device config or the Alpaca setters), so on a freshly imaged SBC ConformU fails at connect and never reaches CheckMethods; a `0` readback is an explicitly set value (null island), not "unset". Before PR #353 the unset site read back as `0`/`0`, ConformU omitted the offset-rate sub-tests ("expected condition at latitudes close to the equator") and aborted CheckMethods with "The highest elevation available ... is below the horizon" (PR #221, run 2). Set the site **in the device config** before the run (the standard rig site is `39.739194, -104.990306, 1609 m`); the Alpaca setters satisfy the guard for the session but are not written back to `registered_devices.json`, so a restart loses them. ConformU's own report will not say `InvalidOperation` — the router reports every failed connect as `NotConnected` — but since #358 the message it shows is the driver's own sentence naming the missing fields, not a bare "Connection failed", so a failing run says what to fix without opening the server log. Confirm the readback:
