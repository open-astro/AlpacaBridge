...
- **ConformU**: HEM27 validated with ConformU 4.3.0 (USB and Wi-Fi); HAE29C validated with ConformU 4.5.0 (USB, 2026-08-25; Park and MoveAxis stop verified against the 1 s asynchronous-initiator target) — 0 errors, 0 issues, 0 timing violations, on Linux arm64. HAE16 EQ validated with ConformU 4.5.0 (USB, 2026-08-25, ConformU running on the Raspberry Pi over localhost) — 0 errors, 0 issues; the two residual ~0.1 s FAST marks on `EquatorialSystem`/`SideOfPier` are ConformU-side .NET first-use JIT of each enum response type (server answers the same requests in 1-2 ms; see the log header note).
